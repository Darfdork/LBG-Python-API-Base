...
class MyLbgApiTestCase(unittest.TestCase):
    ...
    def test_response_contains_expected_json_fields(self):
        ...
        """
        item = requests.post(BASE_URL + '/create', json = {'name': 'Archie', 'description': 'Archie Clark', 'price': 99.9})
        response = requests.get(BASE_URL + '/read/2')
        self.assertEqual(response.json(), {"_id":2, 'name': 'Archie', 'description': 'Archie Clark', 'price': 99.9})
